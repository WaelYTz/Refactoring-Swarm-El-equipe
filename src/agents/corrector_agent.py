    ...
    def run(self, context: "SwarmContext") -> "SwarmContext":
        ...
        try:
            ...
            for file_path in python_files:
                ...
                # ALWAYS skip test files - Corrector should only fix SOURCE code
                # If tests fail, we fix the source based on test feedback, not the tests themselves
                if file_path_obj.name.startswith("test_") or file_path_obj.name.endswith("_test.py"):
                    logger.debug("Skipping test file (Corrector only fixes source code): %s", file_path)
                    continue
